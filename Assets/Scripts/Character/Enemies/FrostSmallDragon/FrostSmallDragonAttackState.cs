...
public class FrostSmallDragonAttackState : BaseState
{
    ...
    {
        //当怪物发现玩家并且触发追击状态，当距离小于0.5的时候，就进入攻击状态
        if (currentEnemy.FoundPlayer() && Mathf.Abs(currentEnemy.transform.position.x - GameObject.FindGameObjectWithTag("Player").transform.position.x) <= 0.5f)
        {
        ...
        if (currentEnemy.FoundPlayer() && Mathf.Abs(currentEnemy.transform.position.x - GameObject.FindGameObjectWithTag("Player").transform.position.x) > 1f)
        {
            Debug.Log("切换到追击状态");
